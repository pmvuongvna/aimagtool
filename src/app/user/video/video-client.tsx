...
type VideoModel = "grok-imagine" | "seedance-2" | "kling-motion-control";
...
export default function VideoClient({ initialPrompt, variant = "grok" }: { initialPrompt: string; variant?: VideoVariant }) {
  ...
  async function onGenerate(e: FormEvent) {
    ...
    const body: CreateTaskInput = videoModel === "kling-motion-control"
      ...
      : videoModel === "seedance-2"
        ? { serviceId: videoModeType === "text" ? "seedance-2-text-video" : "seedance-2-image-video", prompt, aspectRatio: aspectRatio === "auto" ? undefined : aspectRatio, duration: Math.max(1, Math.min(15, duration)), videoResolution: resolution, inputUrl: videoModeType === "image" ? referenceUrl : undefined, nsfwChecker: true }
        : { serviceId: videoModeType === "text" ? "grok-text-video" : "grok-image-video", prompt, aspectRatio: aspectRatio === "auto" ? undefined : aspectRatio, videoMode: mode, duration: Math.max(1, Math.min(30, duration)), videoResolution: resolution === "720p" ? "720p" : "480p", inputUrl: videoModeType === "image" ? referenceUrl : undefined };
  ...
  const modelLabel = videoModel === "kling-motion-control" ? "Kling 2.6" : videoModel === "seedance-2" ? "Seedance 2" : "Grok Imagine";
      ...
      <div className={styles.appShell}>
        ...
        <main className={styles.main} id="dashboard">
            ...
            <form onSubmit={onGenerate}>
              ...
              <div className={styles.controlsCompact} ref={controlsRef}>
                <div className={styles.optionCluster}>
                  <div className={styles.settingDropdown}><button type="button" className={`${styles.settingButton} ${openControl === "model" ? styles.settingButtonActive : ""}`} onClick={() => setOpenControl((prev) => prev === "model" ? null : "model")}><div className={styles.controlSelectIcon}>?</div><div><small>Model</small><strong>{modelLabel}</strong></div></button>{openControl === "model" ? <div className={styles.settingMenu}>{isKlingPage ? <button type="button" className={`${styles.settingMenuItem} ${styles.settingMenuItemActive}`} onClick={() => setOpenControl(null)}>Kling 2.6 Motion Control</button> : <><button type="button" className={`${styles.settingMenuItem} ${videoModel === "grok-imagine" ? styles.settingMenuItemActive : ""}`} onClick={() => { setVideoModel("grok-imagine"); if (resolution === "1080p" || resolution === "4k") setResolution("720p"); setOpenControl(null); }}>Grok Imagine</button><button type="button" className={`${styles.settingMenuItem} ${videoModel === "seedance-2" ? styles.settingMenuItemActive : ""}`} onClick={() => { setVideoModel("seedance-2"); if (duration > 15) setDuration(15); setOpenControl(null); }}>Seedance 2</button><button type="button" className={styles.settingMenuItem} onClick={() => { router.push("/user/kling"); setOpenControl(null); }}>Kling 2.6 Motion Control</button></>}</div> : null}</div>
                  ...
                  <div className={styles.settingDropdown}><button type="button" className={`${styles.settingButton} ${openControl === "quality" ? styles.settingButtonActive : ""}`} onClick={() => setOpenControl((prev) => prev === "quality" ? null : "quality")}><div className={styles.controlSelectIcon}>Q</div><div><small>{videoModel === "kling-motion-control" ? "Output mode" : "Resolution"}</small><strong>{qualityLabel}</strong></div></button>{openControl === "quality" ? <div className={styles.settingMenu}>{videoModel === "kling-motion-control" ? klingModeOptions.map((value) => <button key={value} type="button" className={`${styles.settingMenuItem} ${klingMotionMode === value ? styles.settingMenuItemActive : ""}`} onClick={() => { setKlingMotionMode(value); setOpenControl(null); }}>{value}</button>) : (videoModel === "seedance-2" ? seedanceResolutionOptions : videoResolutionOptions).map((value) => <button key={value} type="button" className={`${styles.settingMenuItem} ${resolution === value ? styles.settingMenuItemActive : ""}`} onClick={() => { setResolution(value); setOpenControl(null); }}>{value}</button>)}</div> : null}</div>
              ...
              {showAdvancedSettings ? (
                <div className={styles.advancedPanel}>
                  ...
                  <div className={styles.advancedPanelGrid}>
                    <div className={styles.fieldBlock}><div className={styles.fieldBlockHeader}><h4>Model AI</h4><span className={styles.fieldHint}>Active pipeline</span></div>{isKlingPage ? <div className={styles.subtleNote}>Kling 2.6 Motion Control</div> : <select value={videoModel} onChange={(e) => { const nextModel = e.target.value as VideoModel; setVideoModel(nextModel); if (nextModel === "grok-imagine" && (resolution === "1080p" || resolution === "4k")) setResolution("720p"); if (nextModel === "seedance-2" && duration > 15) setDuration(15); }}><option value="grok-imagine">Grok Imagine</option><option value="seedance-2">Seedance 2</option></select>}</div>
                    {variant !== "kling" ? (
                      <>
                        ...
                        <div className={styles.fieldBlock}><div className={styles.fieldBlockHeader}><h4>Duration</h4><span className={styles.fieldHint}>{videoModel === "seedance-2" ? "Up to 15 seconds" : "Up to 30 seconds"}</span></div><select value={String(duration)} onChange={(e) => setDuration(Number(e.target.value))}>{durationOptions.filter((value) => videoModel !== "seedance-2" || value <= 15).map((value) => <option key={value} value={value}>{value}s</option>)}</select></div>
          ...
          <section className={styles.statsGrid}>
            ...
            <article className={styles.statCard}><div className={`${styles.statIcon} ${styles.statBlue}`}>M</div><div><small>Current model</small><h3>{videoModel === "kling-motion-control" ? "Kling" : videoModel === "seedance-2" ? "Seedance" : "Grok"}</h3></div><div className={styles.statUp}>+6%</div></article>
